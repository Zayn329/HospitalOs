...
test('MediKiosk AI Clinical Intake Engine API Suite', async (t) => {
  ...
  await t.test('POST /api/v1/medikiosk/session/:id/summary - Multilingual Switching (Marathi, Tamil, English)', async () => {
    // Test Marathi (mr) switching
    const sessionMrRes = await request(app)
      .post('/api/v1/medikiosk/session/start')
      .send({ language: 'mr', mode: 'ayush' })
      .expect(201);
    const mrSessionId = sessionMrRes.body.data.sessionId;

    await request(app)
      .post(`/api/v1/medikiosk/session/${mrSessionId}/questions`)
      .send({ chiefComplaint: 'डोकेदुखी आणि ताप (Headache & Fever)' })
      .expect(200);

    const summaryMrRes = await request(app)
      .post(`/api/v1/medikiosk/session/${mrSessionId}/summary`)
      .expect(200);

    assert.strictEqual(summaryMrRes.body.success, true);
    assert.strictEqual(summaryMrRes.body.data.language, 'mr');
    assert.ok(summaryMrRes.body.data.bilingualAudioConfirmation.patientAudioText.length > 0);
  });
