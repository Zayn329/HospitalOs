...
router.post('/session/:id/ocr', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
    const { fileName, docType, mockOcrText } = req.body;
    const session = sessions.get(id);
    if (!session) {
      return res.status(404).json({ success: false, error: 'Session not found' });
    }
    ...
    const isAbnormal = text.includes('high') || text.includes('elevated') || text.includes('positive') || text.includes('15.2');

    const documentResult = {
      id: `DOC-${Date.now()}`,
      fileName: fileName || 'Scanned_Prescription.jpg',
      docType: docType || 'Prescription',
      extractedDiagnosis: text.includes('diabetes') ? 'Type 2 Diabetes Mellitus' : 'Essential Hypertension',
      ...
      summary: 'Handwritten OCR parsed: Prescribed Metformin & Amlodipine. Elevated HbA1c flagged.'
    };
